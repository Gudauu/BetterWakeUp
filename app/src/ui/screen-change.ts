 ...
 *
 * Home is a stack of screens drawn by swapping what it renders: opening
 * today's walk is a push onto home's own stack, not a navigation the operating
 * system knows about. A sighted user sees the whole screen replaced and needs nothing said.
 ...
 * which screen is now up, and the "Back" control that gets them out is one they
 * have to hunt for.
 *
...
export function screenChangeText(screen: ScreenChange): string {
  return screen.overHome ? `${screen.name}. Back is at the top of the screen.` : `${screen.name}.`;
}
