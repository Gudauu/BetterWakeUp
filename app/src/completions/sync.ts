...
export function createCompletionSync(options: CompletionSyncOptions): CompletionSync {
  ...
  async function attempt(record: PendingCompletionRecord): Promise<CompletionSyncEvent | null> {
    ...
    try {
      ...
      // Whether anything was heard back, which is the only place the difference
      // between "no signal here" and "the server could not answer" survives:
      // both are raised as `internal_error`. A request the client stopped
      // waiting for answers null, and the record then says the true either-way
      // thing rather than blaming a connection that may have been fine.
      const failure = {
        code: error.code,
        message: error.message,
        ...(error.reachedServer === null ? {} : { reachedServer: error.reachedServer }),
      };
      if (error.disposition === "reject") {
        await options.store.markRejected(record.id, failure);
        return { type: "rejected", record, error };
      }
      await options.store.noteAttemptFailed(record.id, failure);
